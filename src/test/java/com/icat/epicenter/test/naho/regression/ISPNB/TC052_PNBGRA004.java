...
public class TC052_PNBGRA004 extends AbstractNAHOTest {

	public TC052_PNBGRA004() {
		super(LoginType.NAHOUSM);
	}

	@Override
	protected String getDataFilePath() {
		return "src/test/resources/testdata/naho/regression/ISPNB/PNBGRA004.xls";
	}
	...
	protected void execute(List<Map<String, String>> data, Map<String, String> setUpData) {
		...
		try {
			...
			Assertions.verify(createQuotePage.namedStormData.getData(), namedStorm, "Create Quote Page",
					"All other risks in LA, outside of Tri County, will have a minimum Named Storm deductible set to "
							+ createQuotePage.namedStormData.getData() + " is verified",
					false, false);
			Assertions.addInfo("Scenario 01", "Scenario 01 Ended");

			// Selecting Personal Injury as $100,000
			if (createQuotePage.personalInjuryArrow.checkIfElementIsPresent()
					&& createQuotePage.personalInjuryArrow.checkIfElementIsDisplayed()) {
				createQuotePage.personalInjuryArrow.scrollToElement();
				createQuotePage.personalInjuryArrow.click();
				createQuotePage.personalInjuryOption.formatDynamicPath(testData.get("PersonalInjury"))
						.scrollToElement();
				createQuotePage.personalInjuryOption.formatDynamicPath(testData.get("PersonalInjury")).click();
			}
			Assertions.passTest("Create quote Page", "Personal Injury selected $100,000 successfully");

			// Selecting Coverage E Value to $500,000
			...
			Assertions.passTest("Create Quote Page", "The Coverage E Value selected $500,000 successfully");

			// Verifying The Personal Injury Drodown Consist $500,000
			createQuotePage.personalInjuryArrow.waitTillPresenceOfElement(60);
			createQuotePage.personalInjuryArrow.waitTillVisibilityOfElement(60);
			createQuotePage.personalInjuryArrow.scrollToElement();
			createQuotePage.personalInjuryArrow.click();

			Assertions.verify(
					createQuotePage.personalInjuryOption.formatDynamicPath(testData.get("L1D1-DwellingCovE"))
							.checkIfElementIsDisplayed(),
					true, "Create Quote Page",
					"When Coverage E selected as  $500,000 the personal injury Drop down has $500,000 displayed is verified",
					false, false);

			// Changing Cov E to None
			testData = data.get(dataValue2);
			createQuotePage.coverageEArrow.scrollToElement();
			createQuotePage.coverageEArrow.click();
			createQuotePage.coverageEOption.formatDynamicPath(testData.get("L1D1-DwellingCovE")).scrollToElement();
			createQuotePage.coverageEOption.formatDynamicPath(testData.get("L1D1-DwellingCovE")).click();
			Assertions.passTest("Create Quote Page", "Coverage E selected as None successfully");

			// Verifying Personal Injury should not display
			Assertions.verify(createQuotePage.personalInjuryArrow.checkIfElementIsPresent(), false, "Create Quote Page",
					"When Coverage E selected as None Personal Injury is not displayed is verified", false, false);
			...
			Assertions.verify(dwellingPage.editBuilding.checkIfElementIsDisplayed(), true, "Dwelling  Page",
					"Dwelling Page Loaded successfully", false, false);
			...
			Assertions.passTest("Dwelling Page", "Clicked on Edit Building");

			// Change the Occupancy as Tenant
			dwellingPage.modifyDwellingDetailsNAHO(testData);
			Assertions.passTest("Dwelling Page", "Selected the Occupany As Tenant successfully");

			// Click on Create Quote
			Assertions.verify(createQuotePage.getAQuote.checkIfElementIsDisplayed(), true, "Create Quote Page",
					"Create Quote Page loaded successfully", false, false);

			// Verifying Personal Injury should not display when Occupancy is Tenant
			Assertions.verify(createQuotePage.personalInjuryArrow.checkIfElementIsPresent(), false, "Create Quote Page",
					"When Occupany selected as Tenant Personal Injury is not displayed is verified", false, false);
			...
			// Adding the ticket IO-21999 for New business
			// Logout as USM
			...
			// Verifying the category drop down has only the subscription agreement option
			// by default for producer
			Assertions.verify(policyDocumentsPage.documentCategoryDataNAHO.getData().contains("Subscription Agreement"),
					true, "Policy Documents Page",
					"The docuement category option " + policyDocumentsPage.documentCategoryDataNAHO.getData()
							+ " selected by default displayed is verified",
					false, false);

			// Click on cancel
			policyDocumentsPage.cancelButtonUAT.scrollToElement();
			policyDocumentsPage.cancelButtonUAT.click();
			// End the ticket IO-21999
			...
			Assertions.verify(createQuotePage.getAQuote.checkIfElementIsDisplayed(), true, "Create Quote Page",
					"Create Quote Page loaded successfully", false, false);
			Assertions.addInfo("Scenario 02", "Verifying minimu default named storm deductible is 3% on renewal quote");
			Assertions.verify(createQuotePage.namedStormData.getData(), namedStorm, "Create Quote Page",
					"All other risks in LA, outside of Tri County, renewal quote will have a minimum Named Storm deductible set to "
							+ createQuotePage.namedStormData.getData() + " is verified",
					false, false);
			...
			// As there is change in functionality for producer don't have a option to upload the document for renewal quotes, so, commenting the below lines
/*
			// Adding the ticket IO-21999 for Renewal
			// Logout as USM
			homePage.signOutButton.scrollToElement();
			homePage.signOutButton.click();
			...
			if (policyDocumentsPage.chooseDocument.checkIfElementIsPresent()
					&& policyDocumentsPage.chooseDocument.checkIfElementIsDisplayed()) {
				policyDocumentsPage.chooseDocument.setData(new File(uploadFileDir + fileName1).getAbsolutePath());
				waitTime(5);// Adding wait time to load the element
				System.out.println("Choose document");
			} else {
				policyDocumentsPage.chooseFile.setData(new File(uploadFileDir + fileName1).getAbsolutePath());
				waitTime(5);// Adding wait time to load the element
...
*/
			...
			Assertions.verify(createQuotePage.getAQuote.checkIfElementIsDisplayed(), true, "Create Quote Page",
					"Create Quote Page loaded successfully", false, false);
			Assertions.addInfo("Scenario 03", "Verifying minimu default named storm deductible is 3% on rewrite quote");
			Assertions.verify(createQuotePage.namedStormData.getData(), namedStorm, "Create Quote Page",
					"All other risks in LA, outside of Tri County, rewrite quote will have a minimum Named Storm deductible set to "
							+ createQuotePage.namedStormData.getData() + " is verified",
					false, false);
...
/*
			...
			waitTime(8);
			String fileName2 = testData.get("FileNameToUpload");
			if (policyDocumentsPage.chooseDocument.checkIfElementIsPresent()
					&& policyDocumentsPage.chooseDocument.checkIfElementIsDisplayed()) {
				policyDocumentsPage.chooseDocument.setData(new File(uploadFileDir + fileName2).getAbsolutePath());
				waitTime(5);// Adding wait time to load the element
				System.out.println("Choose document");
			} else {
				policyDocumentsPage.chooseFile.setData(new File(uploadFileDir + fileName2).getAbsolutePath());
				waitTime(5);// Adding wait time to load the element
...
*/
			...
			Assertions.addInfo("Scenario 04", "Verifying minimum default named storm deductible is 3% as producer");
			Assertions.verify(createQuotePage.namedStormData.getData(), namedStorm, "Create Quote Page",
					"All other risks in LA, outside of Tri County, will have a minimum Named Storm deductible set to "
							+ createQuotePage.namedStormData.getData() + " is verified",
					false, false);
			...
			Assertions.failTest("PNBTC052 ", "Test Failed " + e.getMessage());
		} finally {
			if (isTestPassed) {
				Assertions.passTest("PNBTC052 ", "Executed Successfully");
			}
		}
	}
