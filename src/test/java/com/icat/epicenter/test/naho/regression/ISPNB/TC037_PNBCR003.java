...
public class TC037_PNBCR003 extends AbstractNAHOTest {

	public TC037_PNBCR003() {
		super(LoginType.NAHOUSM);
	}

	@Override
	protected String getDataFilePath() {
		return "src/test/resources/testdata/naho/regression/ISPNB/PNBCR003.xls";
	}
	...
	protected void execute(List<Map<String, String>> data, Map<String, String> setUpData) {
		...
		try {
			...
			// Asserting original premium value
			...
			String originalPremiumData = viewPolicySnapShot.coveragesAndPremiumValues.formatDynamicPath("1").getData();
			int length = originalPremiumData.length();
			String originalPremium = originalPremiumData.substring(8, length).replace("$", "");
			Assertions.passTest("View Policy Snapshot page", "Original premium value is: " + originalPremium);
			viewPolicySnapShot.scrollToTopPage();
			viewPolicySnapShot.waitTime(2);
			viewPolicySnapShot.goBackButton.click();

			// Asserting Premium after flat cancel
			...
			policySummaryPage.viewPolicySnapshot.click();
			Assertions.verify(
					viewPolicySnapShot.coveragesAndPremiumValues.formatDynamicPath("1").getData().replace("$", "")
							.contains(originalPremium),
					true, "View Policy Snapshot Page",
					"Complete premium is returned after flat cancel: " + viewPolicySnapShot.coveragesAndPremiumValues
			...
			// Asserting Premium after reinstate flat cancel
			...
			Assertions.verify(
					viewPolicySnapShot.coveragesAndPremiumValues.formatDynamicPath("1").getData().replace("$", "")
							.contains(originalPremium),
					true, "View Policy Snapshot Page",
					"Complete premium is added back to policy: " + viewPolicySnapShot.coveragesAndPremiumValues
							.formatDynamicPath("1").getData().replace("$", ""),
					false, false);
			viewPolicySnapShot.scrollToTopPage();
			viewPolicySnapShot.waitTime(2);
			...
			// Asserting Premium after non flat cancel
			policySummaryPage.cancelPolicy.waitTillVisibilityOfElement(60);
			policySummaryPage.cancelPolicy.scrollToElement();
			policySummaryPage.cancelPolicy.click();
			...
			Assertions.verify(
					viewPolicySnapShot.coveragesAndPremiumValues
							.formatDynamicPath("1").getData().replace("$", "").contains(originalPremium),
					true, "View Policy Snapshot Page",
					"Percentage premium is returned after non flat cancel: "
							+ viewPolicySnapShot.coveragesAndPremiumValues.formatDynamicPath("1").getData().replace("$",
									""),
					false, false);
			viewPolicySnapShot.scrollToTopPage();
			...
			reinsatePolicyPage.enterReinstatePolicyDetails(testData);
			policySummaryPage.transHistReason.formatDynamicPath("6").waitTillPresenceOfElement(60);
			policySummaryPage.transHistReason.formatDynamicPath("6").scrollToElement();
			policySummaryPage.transHistReason.formatDynamicPath("6").click();
			policySummaryPage.viewPolicySnapshot.waitTillVisibilityOfElement(60);
			policySummaryPage.viewPolicySnapshot.scrollToElement();
			policySummaryPage.viewPolicySnapshot.click();
			Assertions.verify(
					viewPolicySnapShot.coveragesAndPremiumValues.formatDynamicPath("1").getData().replace("$", "")
							.contains(originalPremium),
					true, "View Policy Snapshot Page",
					"Percentage premium is added back to policy: " + viewPolicySnapShot.coveragesAndPremiumValues
							.formatDynamicPath("1").getData().replace("$", ""),
					false, false);
			viewPolicySnapShot.scrollToTopPage();
			viewPolicySnapShot.waitTime(2);
			...
			Assertions.failTest("PNBTC037 ", "Test Failed " + e.getMessage());
		} finally {
			if (isTestPassed) {
				Assertions.passTest("PNBTC037 ", "Executed Successfully");
			}
		}
	}
}
