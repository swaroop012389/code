...
public class TC026_PNBC003 extends AbstractNAHOTest {

	public TC026_PNBC003() {
		super(LoginType.NAHOUSM);
	}

	@Override
	protected String getDataFilePath() {
		return "src/test/resources/testdata/naho/regression/ISPNB/PNBC003.xls";
	}
	...
	protected void execute(List<Map<String, String>> data, Map<String, String> setUpData) {
		...
		try {
			...
			// Get CoverageA value
			String covAValData = createQuotePage.coverageADwelling.getData().replace(",", "");
			double covAVal = Double.parseDouble(covAValData);
			double tenPerCovA = (0.1) * covAVal;

			// Get CoverageC value
			String covCValData = createQuotePage.coverageCPersonalProperty.getData().replace(",", "");
			double covCVal = Double.parseDouble(covCValData);

			// Asserting coverage values
			Assertions.verify(covCVal, tenPerCovA, "Create Quote Page",
					"Coverage C value is 10% of coverageA: $" + createQuotePage.coverageCPersonalProperty.getData(),
					false, false);
			Assertions.verify(createQuotePage.covFValue.getData().contains("None"), true, "Create Quote Page",
					"Coverage F value is: " + createQuotePage.covFValue.getData(), false, false);
			...
			Assertions.failTest("PNBTC026 ", "Test Failed " + e.getMessage());
		} finally {
			if (isTestPassed) {
				Assertions.passTest("PNBTC026 ", "Executed Successfully");
			}
		}
	}
}
