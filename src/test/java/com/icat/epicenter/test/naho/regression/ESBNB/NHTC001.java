/** Program Description: Verifying SLTF values on Quote document and Account overview page are matching, due diligence text, request received messages.
 *  Author			   : Priyanka S
 *  Date of Creation   : 12/14/2021
 **/
...
public class NHTC001 extends AbstractNAHOTest {
	...
	protected String getDataFilePath() {
		return "src/test/resources/testdata/naho/regression/ESBNB/NHTC001.xls";
	}
	...
	protected void execute(List<Map<String, String>> data, Map<String, String> setUpData) {
		...
		try {
			...
			double sltfFee = Double.parseDouble(premiumValue) + Double.parseDouble(policyValue)
			...
			Assertions.verify(
					bindRequestSubmittedPage.requestReceivedMsg.formatDynamicPath("quoted building")
							.checkIfElementIsDisplayed(),
					true, "Bind Request Submitted page",
					"Request Received 1 : " + bindRequestSubmittedPage.requestReceivedMsg
							.formatDynamicPath("quoted building").getData(),
					false, false);
			Assertions.verify(
					bindRequestSubmittedPage.requestReceivedMsg
							.formatDynamicPath("high profile").checkIfElementIsDisplayed(),
					true, "Bind Request Submitted page",
					"Request Received 4 : "
							+ bindRequestSubmittedPage.requestReceivedMsg.formatDynamicPath("high profile").getData(),
					false, false);
			Assertions.verify(
					bindRequestSubmittedPage.requestReceivedMsg
							.formatDynamicPath("non-renewed").checkIfElementIsDisplayed(),
					true, "Bind Request Submitted page",
					"Request Received 7 : "
							+ bindRequestSubmittedPage.requestReceivedMsg.formatDynamicPath("non-renewed").getData(),
					false, false);
			...
			Assertions.failTest("NHTC001 ", "Test Failed " + e.getMessage());
		} finally {
			if (isTestPassed) {
				Assertions.passTest("NHTC001", "Executed Successfully");
			}
		}
	}
}